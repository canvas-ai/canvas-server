...
export default function contextRoutes(options) {
     ...
     * PUT /api/v2/context/:id/initialize
     */
    ...
    /**
     * Document Feature API Routes
     */

    /**
     * Add a document feature to a context
     * POST /api/v2/context/:id/feature
     */
    router.post('/:id/feature', async (req, res) => {
        try {
            ...
            if (!feature) {
                return res.status(400).json(new ResponseObject(null, 'Feature is required', 400));
            }

            const context = contextManager.getContext(id);
            context.addDocumentFeature(feature);

            return res.status(201).json(new ResponseObject({ success: true }));
        } catch (err) {
            debug(`Error adding document feature: ${err.message}`);
            return res.status(500).json(new ResponseObject(null, err.message, 500));
        }
    });

    /**
     * Remove a document feature from a context
     * DELETE /api/v2/context/:id/feature/:feature
     */
    router.delete('/:id/feature/:feature', async (req, res) => {
        try {
            const { id, feature } = req.params;
            const context = contextManager.getContext(id);
            context.removeDocumentFeature(feature);

            return res.json(new ResponseObject({ success: true }));
        } catch (err) {
            debug(`Error removing document feature: ${err.message}`);
            return res.status(500).json(new ResponseObject(null, err.message, 500));
        }
    });

    /**
     * List all document features in a context
     * GET /api/v2/context/:id/feature
     */
    router.get('/:id/feature', async (req, res) => {
        try {
            const { id } = req.params;
            const context = contextManager.getContext(id);
            const features = context.listDocumentFeatures();

            return res.json(new ResponseObject(features));
        } catch (err) {
            debug(`Error listing document features: ${err.message}`);
            return res.status(500).json(new ResponseObject(null, err.message, 500));
        }
    });

    /**
     * Check if a context has a document feature
     * GET /api/v2/context/:id/feature/:feature/check
     */
    router.get('/:id/feature/:feature/check', async (req, res) => {
        try {
            const { id, feature } = req.params;
            const context = contextManager.getContext(id);
            const hasFeature = context.hasDocumentFeature(feature);

            return res.json(new ResponseObject({ feature, exists: hasFeature }));
        } catch (err) {
            debug(`Error checking document feature: ${err.message}`);
            return res.status(500).json(new ResponseObject(null, err.message, 500));
        }
    });
