...
class WorkspaceManager extends Manager {
    ...
    /**
     * Creates a new workspace directory structure, configuration file, and adds it to the index.
     * The workspace ID will be in the format: options.owner/sanitizedName.
     * @param {string} name - The desired name for the workspace.
     * @param {string} [rootPath=this.#rootPath] - The base directory where the workspace will be created. If not provided, the manager's rootPath is used.
     * @param {Object} options - Additional options.
     * @param {string} options.owner - The email address of the user who owns this workspace. **Required**.
     * @param {string} [options.id] - Optional pre-defined ID (overrides generated ID).
     * @param {string} [options.type='workspace'] - Type of workspace ('universe', 'workspace').
     * @param {string} [options.label] - Display label (defaults to capitalized name).
     * @param {string} [options.color] - Hex color code (defaults to random).
     * @param {string} [options.description] - Workspace description.
     * @param {Object} [options.restApi] - REST API configuration ({ port: number, token: string }).
     * @param {boolean} [options.locked=true] - Locked status.
     * @param {Object} [options.acl] - Access Control List ({ [userID]: role }). Owner is added automatically.
     * @returns {Promise<Object>} The index entry of the newly created workspace.
     * @throws {Error} If owner is missing, ID/path conflict, or creation fails.
     */
    async createWorkspace(name, rootPath = this.#rootPath, options = {}) {
        if (!options.owner) {
            throw new Error('options.owner (user email) is required to create a workspace.');
        }
        const owner = options.owner;

        // Use the sanitized name for the filesystem path component and ID generation
        const sanitizedName = WorkspaceManager.sanitizeWorkspaceName(name);

        // Generate a composed ID for the workspace using owner email and sanitized name
        // options.id can override this if explicitly provided
        const workspaceID = options.id || this.#generateComposedWorkspaceId(owner, sanitizedName);

        // Resolve the final path where the workspace directory will reside
        // This might involve joining rootPath, owner, and sanitizedName depending on the desired structure
        // For now, let's assume rootPath defines the *parent* directory where the workspace folder (sanitizedName) will be created.
        // TODO: Refine path resolution logic based on whether rootPath includes the owner segment or not.
        // const workspacePath = await this.#resolveWorkspacePath(rootPath, owner, sanitizedName);
        const workspacePath = path.join(rootPath, sanitizedName); // Simplified for now
        debug(`Resolved workspace path: ${workspacePath}`);


        // Check for unique universe workspace per user (based on ID)
        if (sanitizedName === 'universe') {
            ...
            // If type is not explicitly set to 'universe', set it.
            if (options.type !== 'universe') {
                debug(`Workspace name is 'universe', setting type to 'universe'.`);
                options.type = 'universe';
            }
        }
        ...
        const configData = this.#parseWorkspaceOptions({
            id: workspaceID,
            name, // Use original name for config
            owner, // Ensure owner is passed
            // Ensure owner has 'owner' role in ACL
            acl: { ...options.acl, [owner]: 'owner' },
            ...options, // Spread other options like type, color, etc.
        });
        ...
        // Add to index
        const indexEntry = this.#addWorkspaceToIndex(workspaceID, workspacePath, owner);
        this.emit('workspace:created', indexEntry);
        debug(`Workspace ${workspaceID} created successfully at ${workspacePath}`);
        return indexEntry;
    }
    ...
    #findWorkspaceConfigPath(workspacePath) {
        ...
        const locations = [
            path.join(workspacePath, '.workspace', filename), // $WORKSPACE_ROOT/.workspace/workspace.json
            path.join(workspacePath, `.${filename}`), // $WORKSPACE_ROOT/.workspace.json
            path.join(workspacePath, filename), // $WORKSPACE_ROOT/workspace.json (default)
        ];
    ...
    /**
     * Loads the workspace configuration using the Conf library.
     * Finds the config file using #findWorkspaceConfigPath.
     * @param {string} workspacePath - The root directory of the workspace.
     * @returns {Conf} The Conf instance for the workspace configuration.
    ...
    #loadWorkspaceConfig(workspacePath) {
        ...
        if (!configFilePath) {
            throw new Error(`Workspace config file not found in standard locations for path: ${workspacePath}`);
        }
        const configDir = path.dirname(configFilePath);
        const configName = path.basename(configFilePath, '.json'); // e.g., 'workspace' or '.workspace'
        try {
            return new Conf({ configName, cwd: configDir });
        } catch (error) {
            logger.error(`Failed to load Conf for config '${configName}' in ${configDir}: ${error.message}`);
            throw new Error(`Failed to load workspace configuration at ${configFilePath}: ${error.message}`);
        }
    }
    ...
    async #ensureDirectoryExists(dirPath) {
        await super.ensureDirectoryExists(dirPath); // Use base class method
    }
    ...
    #performInitialScan() {
        ...
        const workspaceIndex = this.
            sArrayIndex(); // Get the array
     ...
     * Retrieves the current array, applies the updater function, and saves it back.
     * @param {function(Array<Object>): Array<Object>} updaterFn - A function that takes the current workspaces array and returns the modified array.
     * @private
     */
    #updateIndex(updaterFn) {
        ...
        // Add comparison logic if needed to avoid unnecessary writes
        // if (JSON.stringify(currentWorkspaces) !== JSON.stringify(updatedWorkspaces)) {
        if (!this.setConfig('workspaces', updatedWorkspaces)) {
            logger.error('#updateIndex: Failed to save updated workspace index.');
            // Depending on severity, might throw an error or just log
            throw new Error('Failed to save workspace index changes.');
        }
        // }
        debug('Workspace index updated successfully.');
    }
     ...
     * @private
     */
    /* REMOVED - Use this.getConfig('workspaces', []) directly */
    /*
    ...
    */
     ...
     * @private
     */
    /* REMOVED - Use #updateIndex helper */
    /*
    ...
    */
    ...
    #findIndexEntry(workspaceID, userID = null) {
        const workspaces = this.getConfig('workspaces', []); // Read directly
        ...
        if (userID) {
            ...
            const nameMatch = workspaces.find(
                (ws) =>
                    ws.owner === userID &&
                    (ws.name === workspaceID || WorkspaceManager.sanitizeWorkspaceName(ws.name) === WorkspaceManager.sanitizeWorkspaceName(workspaceID)),
            );
     ...
     * @param {string} owner - The owner ID for logging/context.
     * @private
     */
    /* REMOVED - Use #updateIndex helper */
    /*
    #addIndexEntry(newEntry, owner) {
        ...
        if (workspaces.some((ws) => ws.id === newEntry.id)) {
            ...
            logger.warn(`Attempted to add duplicate workspace ID "${newEntry.id}" (Owner: ${owner}) to index.`);
            return;
        }
    ...
    */
    ...
    #validateWorkspaceDoesNotExist(workspaceID, workspacePath) {
        ...
        const existingById = this.#findIndexEntry(workspaceID); // Check globally by ID
        if (existingById) {
    ...
    /* Handled via #updateIndex helper (to be added) or direct setConfig */
    /* REMOVED - Use #updateIndex helper */
    /*
    ...
    */
    ...
    async #writeWorkspaceConfig(workspacePath, configData) {
        ...
        // Use the new #loadWorkspaceConfig which finds the correct path/name
        // We need the path *before* creating the Conf instance for writing
        // Let's reuse the logic from #findWorkspaceConfigPath but default to the main file for writing
        let configFilePath = path.join(workspacePath, WORKSPACE_CONFIG_FILENAME); // Default write location
        const possiblePaths = [
            path.join(workspacePath, '.workspace', WORKSPACE_CONFIG_FILENAME),
            path.join(workspacePath, `.${WORKSPACE_CONFIG_FILENAME}`),
            configFilePath
        ];
        // Check if one of the other paths exists, if so, write there instead?
        // This seems complex. For writing a *new* config, let's stick to the default path.
        // If we later need to *update* an existing config, #loadWorkspaceConfig gives the right Conf instance.

        // Create Conf instance pointing to the default config file location
        const configDir = path.dirname(configFilePath);
        const configName = path.basename(configFilePath, '.json');

        // Ensure the directory exists where the config will be written
        // If the config is inside .workspace, that needs creating first
        await this.#ensureDirectoryExists(configDir);

        const workspaceConf = new Conf({ configName, cwd: configDir });
        // Clear existing config before setting new data? No, Conf handles overwrite.
        Object.entries(configData).forEach(([key, value]) => workspaceConf.set(key, value));
        debug(`Wrote workspace config to ${configFilePath}`);
    }
    ...
    /**
     * Resolves the absolute path for a workspace directory.
     * Placeholder implementation. Needs refinement based on how rootPath, owner, and name interact.
     * @param {string} basePath - The base path provided (e.g., manager root or user-specified).
     * @param {string} owner - The workspace owner's ID.
     * @param {string} sanitizedName - The sanitized workspace name.
     * @returns {Promise<string>} The resolved absolute path.
     * @private
     */
    async #resolveWorkspacePath(basePath, owner, sanitizedName) {
        // TODO: Implement actual path resolution logic
        return path.resolve(basePath, sanitizedName); // Simplified placeholder
    }

    /**
     * Creates the standard directories within a new workspace.
     ...
     * @param {string} workspacePath - The root path of the workspace.
     * @param {string} owner - The owner ID for logging/context.
     * @returns {Object} The created index entry.
     * @private
     */
    /* REMOVED - Use #updateIndex helper */
    /*
    #addWorkspaceToIndex(workspaceID, workspacePath, owner) {
        if (!owner) {
            throw new Error('Owner must be provided to add workspace to index');
        }
        const indexEntry = {
            id: workspaceID,
            ...
            restApiStatus: WORKSPACE_API_STATUS.STOPPED,
            pm2Name: null,
            owner: owner, // Use the provided owner
        };
        this.#addIndexEntry(indexEntry, owner);
        return indexEntry;
    }
    */
