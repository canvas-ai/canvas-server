    ...
    async switch(args) {
        try {
            ...
            if (response.data && response.data.status === 'success' && response.data.payload) {
                ...
                console.log(
                    chalk.green(`Switched to context ${contextId} at ${context.workspace || 'unknown'}://${context.url}`),
                );
                return 0;
            } else {
        ...
        try {
            ...
            if (response.data && response.data.status === 'success' && response.data.payload) {
                ...
                bitmaps.forEach((bitmap) => {
                    table.push([chalk.white(bitmap.key || 'N/A'), chalk.white(bitmap.size?.toString() || 'N/A')]);
                });
