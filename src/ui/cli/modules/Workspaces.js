...
import BaseCLI from './BaseCLI.js';

// Import DEFAULT_CONFIG from BaseCLI
import { DEFAULT_CONFIG } from './BaseCLI.js';
  ...
  ${chalk.yellow('--type <type>')}            Workspace type (for create/update)
        ...
        try {
            ...
            if (response.data && response.data.data) {
                ...
                workspaces.forEach((workspace) => {
                    const colorText = workspace.color ? chalk.hex(workspace.color)(workspace.name) : workspace.name;
    ...
    async get(args) {
        try {
            ...
            if (response.data && response.data.data) {
                ...
                table.push(['Name', workspace.color ? chalk.hex(workspace.color)(workspace.name) : workspace.name]);
    ...
    async update(args) {
        try {
            ...
            if (this.args.name) {
                updateData.name = this.args.name;
            }
            if (this.args.description) {
                updateData.description = this.args.description;
            }
            if (this.args.color) {
                updateData.color = this.args.color;
            }
            if (this.args.type) {
                updateData.type = this.args.type;
            }
            if (this.args.status) {
                updateData.status = this.args.status;
            }
