...
class CanvasREPL {
    ...
    getConfigPath() {
        ...
        const configDir = platform === 'win32' ? DEFAULT_CONFIG.paths.windows.configDir : DEFAULT_CONFIG.paths.unix.configDir;
            ...
            this.ws.emit('session:context:list', { sessionId: 'current' }, (response) => {
                if (response.status === 'success' && response.data) {
                    const table = new Table({
                        head: [chalk.cyan('Context ID'), chalk.cyan('URL'), chalk.cyan('Name'), chalk.cyan('Description')],
                        style: {
    ...
    commands = {
        ...
        status: {
            ...
            handler: () => {
                ...
                console.log(
                    `  Current Context: ${this.currentContext ? chalk.green(this.currentContext) : chalk.gray('None')}\n`,
                );
            },
        },
        clear: {
    ...
    generatePrompt() {
        ...
        const serverStatus = this.ws?.connected ? chalk.green('●') : chalk.red('●');

        // Context URL segment
        const contextUrl = this.currentContext ? chalk.cyan(this.currentContext) : chalk.gray('no-context');
