...
async function runTests() {
    ...
    try {
        ws1Meta = await wm.createWorkspace('test-ws-1', { owner: testUserID, description: 'My First Test WS' });
        console.log('Created ws1 (metadata): ', ws1Meta);
        if (!existsSync(ws1Meta.path)) throw new Error(`WS1 directory not created at ${ws1Meta.path}`);
        if (!existsSync(path.join(ws1Meta.path, 'workspace.json'))) throw new Error(`WS1 workspace.json not created`);
        ...
        if (wm.isLoaded(ws1Meta.id, testUserID)) console.error('FAIL: Workspace loaded immediately after creation.');
        else console.log('PASS: Workspace not loaded immediately after creation.');

        wsCustomMeta = await wm.createWorkspace('custom-path-ws', { owner: testUserID, path: customPath });
        console.log('Created custom path ws (metadata): ', wsCustomMeta);
        if (!existsSync(wsCustomMeta.path)) throw new Error(`Custom WS directory not created at ${wsCustomMeta.path}`);
        if (!existsSync(path.join(wsCustomMeta.path, 'workspace.json'))) throw new Error(`Custom WS workspace.json not created`);
        ...
        if (wm.isLoaded(wsCustomMeta.id, testUserID)) console.error('FAIL: Custom workspace loaded immediately after creation.');
        ...
        console.log('Loaded Workspaces after creation: ', wm.listLoadedWorkspaces(testUserID).map(ws => ws.id)); // Should still be empty
    ...
    try {
        await wm.createWorkspace('test-ws-1', { owner: testUserID });
     ...
     try {
        await wm.createWorkspace('another-ws-custom', { owner: testUserID, path: customPath });
    ...
    let ws1Loaded;
    try {
        ws1Loaded = await wm.openWorkspace(ws1Meta.id, testUserID);
        ...
        if (!wm.isLoaded(ws1Meta.id, testUserID)) throw new Error('ws1 not marked as loaded after open.');
        ...
        const ws1Status = wm.getWorkspaceStatus(ws1Meta.id, testUserID);
    ...
    try {
        const ws1Again = await wm.openWorkspace(ws1Meta.id, testUserID);
    ...
    let ws1Active;
    try {
        ws1Active = await wm.startWorkspace(ws1Meta.id, testUserID);
        ...
        const ws1StatusIndex = wm.getWorkspaceStatus(ws1Meta.id, testUserID);
    ...
    // Test starting already active (should return same instance)
     try {
        const ws1Again = await wm.startWorkspace(ws1Meta.id, testUserID);
    ...
    try {
        const startedNonExistent = await wm.startWorkspace('non-existent-ws', testUserID);
    ...
    try {
        let success = await wm.setWorkspaceProperty(ws1Meta.id, testUserID, 'label', 'My Test Label');
        ...
        success = await wm.setWorkspaceProperty(ws1Meta.id, testUserID, 'color', '#123456');
        ...
        const ws1ConfigRaw = await fs.readFile(path.join(ws1Meta.path, 'workspace.json'), 'utf8');
    ...
    try {
        const stopped = await wm.stopWorkspace(ws1Meta.id, testUserID);
        ...
        console.log('WS1 instance status: ', ws1Active.status); // Check instance status
     ...
     try {
        const stoppedAgain = await wm.stopWorkspace(ws1Meta.id, testUserID);
        if (!stoppedAgain) {
    ...
    try {
        ...
        const stoppedNonExistent = await wm.stopWorkspace(nonExistentId, testUserID);
    ...
    try {
        const closed = await wm.closeWorkspace(ws1Meta.id, testUserID); // ws1 is currently stopped
        if (!closed) throw new Error('closeWorkspace returned false for stopped workspace');
        console.log('Closed ws1 successfully.');
        if (wm.isLoaded(ws1Meta.id, testUserID)) throw new Error('Workspace still loaded after close.');
        console.log('Loaded workspaces after close: ', wm.listLoadedWorkspaces(testUserID).map(ws => ws.id)); // Should be empty
        console.log('WS1 index status after close: ', wm.getWorkspaceStatus(ws1Meta.id, testUserID)); // Should remain INACTIVE
    } catch (err) {
         console.error('Error during closeWorkspace: ', err);
    }
    // Test closing already closed
     try {
        const closedAgain = await wm.closeWorkspace(ws1Meta.id, testUserID);
    ...
    try {
        let success = await wm.setWorkspaceProperty(ws1Meta.id, testUserID, 'description', 'Updated Description');
    ...
    try {
        ...
        exportPath = await wm.exportWorkspace(ws1Meta.id, testUserID, exportDir);
    ...
    try {
         wsCustomLoaded = await wm.openWorkspace(wsCustomMeta.id, testUserID);
         wsCustomActive = await wm.startWorkspace(wsCustomMeta.id, testUserID);
         console.log('Opened and started custom ws for export test.');
         await wm.exportWorkspace(wsCustomMeta.id, testUserID, exportDir);
        ...
        if (wsCustomActive) await wm.stopWorkspace(wsCustomMeta.id, testUserID);
        if (wsCustomLoaded) await wm.closeWorkspace(wsCustomMeta.id, testUserID);
    }
    ...
    try {
        ...
        if (wm.isLoaded(ws1Meta.id, testUserID)) {
            await wm.closeWorkspace(ws1Meta.id, testUserID); // Close it first if loaded
        }
        const destroyedOriginal = await wm.destroyWorkspace(ws1Meta.id, testUserID);
        ...
        if (!existsSync(importedWsMeta.path)) throw new Error(`Imported WS directory not created at ${importedWsMeta.path}`);
        const importedStatus = wm.getWorkspaceStatus(importedWsMeta.id, testUserID);
        ...
        if (wm.isLoaded(importedWsMeta.id, testUserID)) {
        ...
        const importedWsOpened = await wm.openWorkspace(importedWsMeta.id, testUserID);
        if (!importedWsOpened) throw new Error('Failed to open imported workspace.');
        console.log('Opened imported workspace.');
        const importedWsActive = await wm.startWorkspace(importedWsMeta.id, testUserID);
        ...
        await wm.stopWorkspace(importedWsMeta.id, testUserID);
        await wm.closeWorkspace(importedWsMeta.id, testUserID);
    ...
    try {
        ...
        const importedPath = wm.index[importedId].path; // Get path before removing

        const removed = await wm.removeWorkspace(importedId, testUserID);
    ...
    try {
        ...
        const destroyMeta = await wm.createWorkspace('to-be-destroyed', { owner: testUserID });
        ...
        const destroyOpened = await wm.openWorkspace(destroyId, testUserID);
        const destroyActive = await wm.startWorkspace(destroyId, testUserID);
        console.log(`Opened and started workspace ${destroyId} before destroy test.`);

        const destroyed = await wm.destroyWorkspace(destroyId, testUserID);
